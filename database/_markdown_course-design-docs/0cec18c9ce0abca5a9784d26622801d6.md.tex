...
{}\begin{outline} \1 user(\underline{id}, nickname, username, email, password, email_verified, created_at, updated_at) \1 role(\underline{id}, name, target) \1 user\markdownRendererEmphasis{has}roles(\underline{id}, user_id, ) \1 token(\underline{id},) \end{outline}\relax