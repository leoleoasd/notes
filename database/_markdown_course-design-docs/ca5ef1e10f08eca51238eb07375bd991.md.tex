...
\markdownRendererUlItem problems(\underline{id}, name, description, attach\_file\_name, public, memory\_limit, time\_limit, language\_allowed, compare\_script\_name, build\_arg)\markdownRendererUlItemEnd 
\markdownRendererUlEndTight \markdownRendererInterblockSeparator
...
{}表中所有关系模式中均依赖主键\underline{id}，符合范式。\markdownRendererInterblockSeparator
...
{}表中所有关系模式均只有一个主属性，不存在其他键码，同时非主属性也依赖与键码，所以符合范式。\relax